    ...
    try:
        with open(conf.hist_file, "r") as f:
            for line in f:
                ...
                ip = times.pop(0)
                history[ip] = [
                    datetime.datetime.strptime(t, datetime_fmt) for t in times
                ]
...
def check_hist(ip):
    ...
    if ip in hist:
        ...
        if len(hist[ip]) > 2 * conf.hist_max_uses:
            ban(ip)
            raise SecurityError(
                "Your ip has been banned from using this service. If you feel this is not warranted, please contact this service's operator"
            )
        if len(hist[ip]) > conf.hist_max_uses:
            raise SecurityError(
                "In order to prevent abuse of this service, you can only send a limited number of emails at a given time. Please try again later. If you persist in trying to use this interface too frequently, you will eventually get banned from this service."
            )
    else:
